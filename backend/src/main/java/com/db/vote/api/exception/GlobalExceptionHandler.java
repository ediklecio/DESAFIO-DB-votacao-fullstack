...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ErrorResponse> handleInvalidCpf(InvalidCpfException exception) {
		log.warn("Rejected invalid CPF: {}", exception.getMessage());
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ErrorResponse(exception.getMessage()));
	}

	@ExceptionHandler(UnableToVoteException.class)
	public ResponseEntity<ErrorResponse> handleUnableToVote(UnableToVoteException exception) {
		log.warn("Rejected vote from a member unable to vote: {}", exception.getMessage());
		return ResponseEntity.unprocessableEntity().body(new ErrorResponse(exception.getMessage()));
	}
}
